...
// HAL_GetTick() for apogee confirmation timing.
// Already included by state_machine.h — no extra cost.
// #if F4
...
private:
    ...
    double  calib_sum_mag_ = 0.0;  // running sum of per-sample magnitudes
    ...
    void update_apogee_confirmation(float velocity, float altitude);

    // ── Tuned noise constants (from flights FL001 + FL002) ────────────────────
    static constexpr double Q_POS    = 0.1;
    static constexpr double Q_VEL    = 0.5;
    static constexpr double Q_BIAS   = 1e-4;
    static constexpr double R_COAST  = 0.3;
    static constexpr double R_THRUST = 500.0;
    static constexpr double R_DROGUE = 10.0;
    static constexpr double R_MAIN   = 2.0;
};
